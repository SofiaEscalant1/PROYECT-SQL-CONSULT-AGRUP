...
SELECT fecha, SUM(cantidad) AS total_insicritos_dia FROM INSCRITOS GROUP BY fecha ORDER BY fecha ASC;

/* 5. ¿Cuántos inscritos hay por fuente? */
SELECT fuente, SUM(cantidad) AS total_inscritos_fuente FROM INSCRITOS GROUP BY fuente;

/*6. ¿Qué día se inscribió la mayor cantidad de personas? Y ¿Cuántas personas se
inscribieron en ese día?*/
SELECT fecha, SUM(cantidad) AS max_inscritos_dia FROM inscritos GROUP BY fecha ORDER BY max_inscritos_dia DESC LIMIT 1;
...
SELECT fecha, AVG(cantidad) AS prom_inscritos FROM INSCRITOS GROUP BY fecha ORDER BY fecha ASC;
